...
/*
 * Arguments    : const double gridIn[1536]
 *                creal_T yout[1536]
 * Return Type  : void
 */
void modulateOpt(const double gridIn[1536], creal_T yout[1536])
{
  ...
  /*  IFFT shift     */
  memcpy(&x[0], &gridIn[0], 1536U * sizeof(double));
  for (k = 0; k < 768; k++) {
    tmp = x[k];
    x[k] = x[k + 768];
    x[k + 768] = tmp;
  ...
  for (k = 0; k < 1536; k++) {
    ...
    d1 = wwc[k + 1535].im;
    tmp = postShift[k].re;
    twid_im = postShift[k].im;
    yout[k].re = d * tmp + d1 * twid_im;
    yout[k].im = d * twid_im - d1 * tmp;
  }
  ...
  while (k > 0) {
    ...
    for (j = k; j < 2048; j += k) {
      ...
      while (i < ihi) {
        nt_re_tmp = i + rt;
        nt_re = tmp * fy[nt_re_tmp].re - twid_im * fy[nt_re_tmp].im;
        nt_im = tmp * fy[nt_re_tmp].im + twid_im * fy[nt_re_tmp].re;
        fy[nt_re_tmp].re = fy[i].re - nt_re;
        fy[nt_re_tmp].im = fy[i].im - nt_im;
        fy[i].re += nt_re;
        fy[i].im += nt_im;
  ...
  while (k > 0) {
    ...
    for (j = k; j < 2048; j += k) {
      ...
      while (i < ihi) {
        nt_re_tmp = i + rt;
        nt_re = tmp * fv[nt_re_tmp].re - twid_im * fv[nt_re_tmp].im;
        nt_im = tmp * fv[nt_re_tmp].im + twid_im * fv[nt_re_tmp].re;
        fv[nt_re_tmp].re = fv[i].re - nt_re;
        fv[nt_re_tmp].im = fv[i].im - nt_im;
        fv[i].re += nt_re;
        fv[i].im += nt_im;
  ...
  for (k = 0; k < 1536; k++) {
    ...
    nt_re = d * tmp + d1 * twid_im;
    tmp = d * twid_im - d1 * tmp;
    if (tmp == 0.0) {
      yout[k].re = nt_re / 1536.0;
      yout[k].im = 0.0;
    } else if (nt_re == 0.0) {
      yout[k].re = 0.0;
      yout[k].im = tmp / 1536.0;
    } else {
      yout[k].re = nt_re / 1536.0;
      yout[k].im = tmp / 1536.0;
    }
...
/*
 * File trailer for modulateOpt.c
 *
 * [EOF]
 */
