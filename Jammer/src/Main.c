...
//OAI Parameters 
#define MAX_PRB 106
#define PUSCH_SYM_LENGTH 13

//5G Standard
#define SUB_PER_PRB 12

//USRP Constants
#define USRP_PACKET_SIZE 1916
#define USRP_IQ_PACKET_SIZE 3832
...
float sine_data[1274][49816];
float jamming_signals[5565][49816];
int Lrb_idx[106];
float txWaveForm[63415768];
float tx_sine_buff[49816];
...
/*
 * Filters noise from UE detection
 */
void* filter_thread(){
	while(done != 0){
		sem_wait(&filter);
	...
	while (done != 0){
		...
		if(done!=0){
			if (dcibits[0] == 0){
				...
				// Decode RIV to Obtain Lrb RBstart
				for (i = 0; i < Nrb; i++){
					for (j = 0; j < (Nrb - i); j++){
						if ((i - 1) <= (Nrb / 2)){
							if (RIV == Nrb * (i - 1) + j){
				...
				if(RIV_count == 1){
					...
					printf("Skipped PUSCH... Multiple RIV\n");
				}
				// printf("RIV Count: %d\n", RIV_count);
				// printf("RBstart: %d\n", RBstart);
				// printf("Lrb: %d\n", Lrb);
...
void init_sine_data(){
	...
	for (y = 0; y < 1273; y++){
		for (x = 0; x < 19160; x++){
			sine_data[y][x] = txWaveForm[x + z];
		}
		z += 19160;
	}

	for (h = 1; h < MAX_PRB; h++){ //h represents number of PRBs allocated
		for (i = 0; i < (MAX_PRB-h); i++){ //i represents the starting resource block
			for (j = 0; j < (i*SUB_PER_PRB); j++){ //j represents the required number of subcarriers
				for (k = 0; k < (USRP_IQ_PACKET_SIZE*PUSCH_SYM_LENGTH); k++){ //k represents the index of the sine samples
					if (j == 0){ //reads first subcarrier in
						jamming_signals[i+a][k] = sine_data[i+j][k]/(SUB_PER_PRB*PUSCH_SYM_LENGTH);
					}
					else{ //adds remaining subcarriers to initial subcarrier
						jamming_signals[i+a][k] += sine_data[i+j][k]/(SUB_PER_PRB*PUSCH_SYM_LENGTH);
					}
				}
			}
		}
		a += i; //a is used to allign the rows of the jamming signal array
		Lrb_idx[h-1] = a; //row corresponding to the first starting resource block of the current PRB allocation
	}
}
