...
function wavegenConfig = hSIB1WaveformConfiguration(config)
    ...
    % the MIB content in SS burst. The BWP spans exactly CORESET0. The
    % frequency offset between CORESET0 and SS burst is determined by
    % the parameter PDCCHConfigSIB1 in MIB as defined in TS 38.213
    % Tables 13-1 through 13-10. CORESET0 is located in a multiple of 6
    % RB from CRB0.
    ...
    % Configure CORESET0 and Search Spaces
    ...
    % Set other parameters of CORESET0 according to TS 38.211 Section 7.3.2.2
    ...
    dci.RIV = hRIV(csetNRB,0,8);    % NSizeBWP,RBStart,LRBS associated to PDSCH
    dci.TDDIndex = 0;               % Time domain resource allocation index (row index - 1) in TS 38.214 Table 5.1.2.1.1-2
    dci.VRBToPRBInterleaving = 0;   % Virtual resource blocks to physical resource blocks interleaving (1/0)
    dci.ModCoding = 0;              % Modulation and coding scheme (0-9 for QPSK)
    dci.RV = 0;                     % Redundancy version (0,1,2,3)
    dci.SIIndicator = 0;            % System information indicator. 0 for SIB1 (TS 38.212 Table 7.3.1.2.1-2)
    dci.Reserved = 0;
    ...
    pdcch = getWavegenPDCCHConfig(csetNRB,ss,dci,enabledPDCCH);
...
function [carrier,bwp] = getCommonSCSCarrierAndBWPConfig(ssb,minChanBW)
    ...
    % Set the SCS carrier bandwidth to include CORESET0
    ...
    % CORESET0 frequency offset from carrier center and carrier size
...
function ssBurst = getWavegenSSBurstConfig(ssb)
    ...
    % of CORESET0 and PDCCH. The 4 MSB configure CORESET0 resources
...
function pdcch = getWavegenPDCCHConfig(csetNRB,ss,dci,enable)
    ...
    dcispec1_0 = hSystemInformationDCIFieldsSize(csetNRB);
    pdcch(1:length(ss)) = nrWavegenPDCCHConfig;

    for ch = 1:length(dci)
        dcibits = hDCI2bits(dcispec1_0,dci(ch));
        ...
        pdcch(ch).DataBlockSize = length(dcibits);
    ...
    % DCI structure to bit vector
    function dcibits = hDCI2bits(dcispec,dci)
        fieldsizes = structfun(@(x)x,dcispec);
        x = cellfun(@(x,y)dec2bin(x,y), struct2cell(dci), num2cell(fieldsizes), 'UniformOutput', false);
        dcibits = uint8([x{:}]=='1');
    end
end

% Configure SIB1 PDSCH. The length of the input structure dci, slotPeriod
% and enabled must be equal to the number of SSB. 
function pdsch = getWavegenPDSCHConfig(csetNRB,csetPattern,dmrsTypeAPosition,dci,ssSlotPeriod,enable)
    ...
    for ch = 1:nch
        ...
        pdsch(ch).TargetCodeRate = hMCS(dci(ch).ModCoding);
        pdsch(ch).XOverhead = 0; % TS 38.214 Section 5.1.3.2
        pdsch(ch).RVSequence = dci(ch).RV;
