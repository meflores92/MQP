...
function [rxWaveform, freqOffset, NID2] = hSSBurstFrequencyCorrect(rxWaveform,rxSampleRate) 
   %# codgen 
    
    syncOfdmInfo = nrOFDMInfo(20, 30,'SampleRate',7680000,'Nfft',256);
    ...
    refGrid = zeros([20*12 2 3]);
            
    % Frequency offset and PSS search
    fshifts = (-180:30:180) * 1e3/2; % Half subcarrier step
    ...
    for fIdx = 1:13
        ...
        rxWaveformDS = resample(rxWaveformFreqCorrected,7680000,rxSampleRate);
        
        for NID2 = [0 1 2]
            refGrid(kPSS,2,NID2+1) = nrPSS(NID2);
            
            nSlot = 0;
            [~,corr] = nrTimingEstimate(rxWaveformDS,20,30,nSlot,refGrid(:,:,NID2+1),'SampleRate',7680000,'Nfft',256);
            corr = sum(abs(corr),2);
...
%     xlabel('Frequency Offset (kHz)');
    ...
    coarseFrequencyOffset = fshifts(fIdx);
    coarseFrequencyOffset = coarseFrequencyOffset(1);
    NID2 = NID2 - 1;
...
%     legend([legends "coarse $\Delta_f$ = " + num2str(coarseFrequencyOffset) + ", $N_{ID}^{(2)}$ = " + num2str(NID2)],'Location','East');
    ...
    waveform = rxWaveformDS(1+offset:end,:);
    ...
    fineFrequencyOffset =  scs * angle(mean(y(cpCorrIndex,:))) / (2*pi);
