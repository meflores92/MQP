function [rx_payload, crc_result]  = uci_decode(encoded_uci, K, N, E, I_seg, I_BIL, q_info_list, q_pc_list, crc_length, payload_size)
...
for code_block_index = 1:code_block_number
    ...
    for x = 1:100
    tic
    rx_code_block_tilt{1} = polar_scl_decode(32*de_rate_matched_bits{code_block_index}, N, info_pattern_indicator, crc_length, path_number, P2, K, PC_bit_pattern, frozen_bits_indicator);
    a =toc;
    sum = sum+a;
    end
    
    sum/100
%     if isequal(rx_code_block{code_block_index}, rx_code_block_tilt{1})
%       disp('SC and SCL decoding get the same result.');
%     else
%       disp('SC and SCL decoding get different results.');
%     end    
% %     
    crc_result = crc_for_5g(rx_code_block{code_block_index}, '24C');
