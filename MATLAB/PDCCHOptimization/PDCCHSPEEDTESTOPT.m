%#codgen
function PDCCHSPEEDTESTOPT(RNTI, in)
...
[allInd,allDMRS,allDMRSInd] = nrPDCCHSpaceOpt(carrier,pdcch);
...
rxCarrGrid = nrOFDMDemodulateOpt(rxWaveN,info,carrier.NSlot, 14, 1);

% Channel estimate
[hest,nVar] = nrChannelEstimateOpt(carrier,rxCarrGrid,cDMRSInd,cDMRS);
[rxSym,pdcchHest] = nrExtractResourcesOpt(cSymIdx,rxCarrGrid,hest);

% Equalization
[pdcchEq,csi] = nrEqualizeMMSEOpt(rxSym,pdcchHest,nVar);



rxCW = nrPDCCHDecodeOpt(pdcchEq,0,RNTI,nVar);

[~,errFlag] = nrDCIDecodeOpt(rxCW,39,listLen, RNTI);
% Decode
